...
module OutputTest = struct
  ...
  let test_run {name; dfn; cmd_list; io_list} =
    name >:: (fun _ ->
      ...
      let ipt_list, expected_opt_list = List.split io_list in
      let state_list = ipt_list |> List.map (fun ipt -> Bf.run bf (String.enum ipt)) in
      let opt_list = state_list |> List.map (fun state -> Bf.State.output_to_string state) in
      assert_equal ~printer:(String.concat "/") expected_opt_list opt_list;
      assert_bool "err" (state_list |> List.for_all (fun Bf.State.{ err; _ } -> err = None))
    )
